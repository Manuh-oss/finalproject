    ...
    getStudentSessions((allSessions) => {
      allInputs.forEach(input => {
        ...
        if(session){
           ...
           const schoolSubjects = schoolDatum.subjects;

           if(editMode !== null && editMode.id === session.id){
              input.value = "";
           }else{
            let savedSubject = session.subject;
            const index = myDefaultSubjects.findIndex(
                (subj) => subj.toLowerCase().trim() === savedSubject.toLowerCase().trim()
            );//get the index of saved subject from my default subjects array
          
              input.value = 
              session.subject+"-"+session.teacher+"-"+session.type; //post the usual subject
  ...
  allInputs.forEach((input) => {
    input.addEventListener("change", (e) => {
      ...
      if (verified) {
        ...
        const correctedSubject = (value);
        const index = schoolSubjects.findIndex(
          subj => subj.toLowerCase().trim() === correctedSubject.toLowerCase().trim()
        );
        const actualSubject = myDefaultSubjects[index];
        const subjectInitial = getInitials(correctedSubject.toLowerCase());
        getSubjectTeacher(actualSubject , (subjectTeacher) => {
            ...
            if (subjectTeacher) {
              checkUserAvailability(subjectTeacher,input.id , (available) => {
                ...
                if(available){
                    ...
                    input.value = 
                      actualSubject+"-"+subjectTeacher.firstname+"-"+type; 
                    const lessonInfo = {
  ...
  getTeacherSessions((teacherSessions) => {
    ...
    allInputs.forEach(input => {
      if(input.id === id){
        ...
        const span = document.createElement("span");
        if(type){
          let currentSubject = lessonInfo.subject
          const index = schoolSubjects.findIndex(
            (subj) => subj.toLowerCase().trim() === currentSubject.trim().toLowerCase()
          );
            ...
            if(lessonInfo.type === "d"){
              ...
              parent.appendChild(span);
              input.value =
              myDefaultSubjects[index]+"-"+clas+"-"+stream+"-"+"d";
              ...
              parent.appendChild(span);
              input.value =
              myDefaultSubjects[index]+"-"+clas+"-"+stream+"-"+"s";
            }else{
              status = false;
            }
        }else{
...
function lessonUlFunction(input,lessons,lessonInfo){
  ...
  if(myLessons.length > 0){
      ...
      const schoolSubjects = schoolDatum.subjects;//this gets school submitted subjects;
      myLessons.forEach(lesson => {
        const li = document.createElement("li");
        let savedSubject = lesson.subject;
        const index = myDefaultSubjects.findIndex(
                (subj) => subj.toLowerCase().trim() === savedSubject.toLowerCase().trim()
        ); // gets index of database subject so i can diaply school subject
        li.innerHTML = `
            <h4>${lesson.class} 
                ${(lesson.stream)} ${schoolSubjects[index]}
            </h4>
        `;
        ul.appendChild(li);

        li.addEventListener("click" , (e) => {
          let savedLiSubject = lesson.subject;
          const liIndex = myDefaultSubjects.findIndex(
                  (subj) => subj.toLowerCase().trim() === savedLiSubject.toLowerCase().trim()
          ); // gets index of database subject so i can diaply school subject
          e.stopPropagation();
          clas = lesson.class
          ...
          span.innerHTML = `
             <h3>${schoolSubjects[liIndex]}</h3>
             <h4>${lesson.class} ${(lesson.stream)}</h4>
          `; 
